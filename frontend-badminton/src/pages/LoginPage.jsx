...
export default function LoginPage() {
  ...
  useEffect(() => {
    document.title = "Login Admin - GOR WIJAYA";
    
    // Membersihkan sisa mode gelap secara menyeluruh
  ...
  const handleLogin = (e) => {
    ...
    .then((response) => {
      ...
      // Menyimpan preferensi ingat akun
      if (rememberMe) {
    ...
    .catch((err) => {
      setIsLoading(false);
      setError("Login Gagal! Username atau kata sandi salah.");
      console.error(err);
    });
  };
  ...
  const styles = {
    pageBg: {
      ...
      // 👇 PERBAIKAN 1: Background halaman diubah jadi abu-abu super muda agar Card putihnya kontras 👇
    ...
    card: {
      ...
      // 👇 PERBAIKAN 2: Efek bayangan dipertajam agar kotak benar-benar terlihat melayang dan berpisah dari background 👇
    ...
    checkboxLabel: { display: 'flex', alignItems: 'flex-start', gap: '10px', fontSize: '0.85rem', color: '#4b5563', cursor: 'pointer', lineHeight: '1.4' }
  };
    ...
    <div style={styles.pageBg} className="auth-wrapper-secure">
      ...
      <div style={styles.card}>
        <div style={{ textAlign: 'center', marginBottom: '30px' }}>
          ...
          <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: '0 0 8px 0' }}>Halaman Login</h2>
          <p style={{ margin: 0, fontSize: '0.9rem', color: '#6b7280' }}>Silakan masuk untuk mengelola sistem.</p>
        </div>
        ...
        <form onSubmit={handleLogin} style={{ margin: 0 }}>
          ...
          <button type="submit" disabled={isLoading} style={{ ...styles.btnAction, ...(isLoading ? styles.btnActionDisabled : {}) }}>
            {isLoading ? '⏳ Memeriksa...' : 'Login'}
          </button>
        </form>

        <div style={{ textAlign: 'center', marginTop: '25px', fontSize: '0.9rem' }}>
          <span style={{ color: '#4b5563' }}>Belum memiliki akun?</span>
          {' '}
          <Link to="/register" className="link-toggle" style={styles.linkToggle}>Register</Link>
        </div>
      </div>
    </div>
  );
